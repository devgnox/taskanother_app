...
const ListHeader = ({ data,onAddCard }: IListHeaderProps) => {
  ...
  const handleSubmit = (formData:FormData) => {
    ...
    execute({title:titleInput, id:listId, boardId});
  };
      ...
      {isEditing ? (
        <form action={handleSubmit} ref={formRef} className="flex-1 px-[2px]">
          <FormInput
            ...
            defaultValue={title}
            onChange={()=>{}}
            className="text-sm px-[7px] py-1 h-7 font-medium border-transparent hover:border-input truncate bg-transparent focus:bg-white focus:border-input transition"
          />
